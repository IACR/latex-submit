...
def test_bibtex_style():
    ...
    assert len(compilation.bibhtml) == 24
...
#    assert _bstrip(compilation.bibhtml[1].body) == 'Philip Bernstein, V. Hadzilacos, and N. Goodman. Concurrency Control and Recovery in Database Systems, chapter 7-8, pages 217–304. Addison-Wesley, 1987.'
    ...
    assert _bstrip(compilation.bibhtml[5].body) == 'Jintai Ding and Dieter Schmidt.  <em>Solving Degree and Degree of Regularity for Polynomial Systems over a Finite Fields</em>. , page 34–49. Springer, Berlin, Heidelberg 2013.'
...
#    assert _bstrip(compilation.bibhtml[8].body) == 'Oded Goldreich.  <em>Foundations of Cryptography: Basic Applications</em>, volume 2. Cambridge University Press, Cambridge, UK 2004.'
    ...
    assert _bstrip(compilation.bibhtml[12].body) == 'Ueli M. Maurer.  Secure multi-party computation made simple.  <em>Discrete Applied Mathematics</em>, 154(2):370–381, 2006. DOI: <a href="https://doi.org/10.1016/j.dam.2005.03.020">10.1016/j.dam.2005.03.020</a>'
#    assert _bstrip(compilation.bibhtml[13].body) == 'Kevin S McCurley.  <em>Odds and ends from cryptology and computational number theory</em>, volume 42 of <em>Proceedings of Symposia in Applied Mathematics</em>, page 49–74. American Mathematical Society 1990.'
    ...
    assert _bstrip(compilation.bibhtml[18].body) == 'Peter Schwabe, Roberto Avanzi, Joppe Bos, Léo Ducas, Eike Kiltz, Tancrède Lepoint, V adim Lyubashevsky, John M. Schanck, Gregor Seiler, Damien Stehlé, and Jintai Ding.  CRYSTALS-KYBER. Technical report, National Institute of Standards and Technology.  available at <a href="https://csrc.nist.gov/Projects/post-quantum-cryptography/selected-algorithms-2022">https://csrc.nist.gov/Projects/post-quantum-cryptography/selected-algorithms-2022</a>. 2022.'
#    assert _bstrip(compilation.bibhtml[19].body) == 'John Smythe and Jane Doer.  The Effects of Hemlength Change.  In Bob Johnson, editor,  <em>Proceedings of the Annual Conference on Climate Change</em>, pages 55–62, Los Angeles, CA. June 2022.  Springer.'
    assert _bstrip(compilation.bibhtml[20].body) == 'John Smith.  An Analysis of Example.  In Jane Doe, editor,  <em>The Book of Examples</em>, volume 1 of <em>Example Book Series</em>, pages 123–130. Example Publisher, New York, NY 2022.'
    assert _bstrip(compilation.bibhtml[21].body) == 'Anton Stolbunov.  <em>Cryptographic schemes based on isogenies</em>. PhD thesis, Norges teknisk-naturvitenskapelige universitet, Fakultet for informasjonsteknologi, matematikk og elektroteknikk, Institutt for telematikk, 2012.'
    assert _bstrip(compilation.bibhtml[23].body) == 'Fester Zester.  My life story in 25 words or less. Available at <a href="https://eprint.iacr.org/2015/939.pdf">https://eprint.iacr.org/2015/939.pdf</a>. May 2015.'
